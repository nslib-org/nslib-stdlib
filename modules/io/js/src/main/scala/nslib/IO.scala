...
/** File I/O utilities — Scala.js stub.
  *
  * Full file-system IO is not yet implemented for Scala.js. The JVM platform provides the complete
  * implementation.
  *
...
object IO {
  ...
  def writeString(path: String, content: String, charset: String): Unit = notSupported(
    "writeString"
  )
